...
#Added and tweaked the functions to work for the multi tier nodes( paret child nodes)
#The code can perform link failure and end to end packet delay together


##########################################################
#SCRIPT PREREQUISITE
#########################################################
# 1. This scipt calculates the end to end packet travel time
# 2. Create a multi node topology with the parent child compbination
#    in the folloowing order. If in case you want to change the Tier, modify the list mnlrTierAddress.
#       (1.1,1.2,1.3,2.1.1,2.1.2, 2.2.1,[2.2.2,2.3.1],2.3.4,2.3.3,3.1.1.1,[3.1.1.2,3.1.2.1],3.2.1.1,[3.2.2.1,3.3.1.1],[3.3.2.1,3.3.3.1],3.3.3.2)
# 3. Specify the code path( in the emulab linux instance)
# 4. Specify the number of nodes(can scale up by adding the tier to the mnlrTierAddress list)
# 5. Number of IP nodes(which is always going to be 2)
# 6. Specify the endnode IDS (endNWNodeIDS)and endnode IP's(endNWIPS)
#==========================================================================
#
#
#
#
###########################################################################
#The script automatically allocates the MNLR Tier address 
#based on the number of nodes specified. It Runs the tshark on the interface 
# and the .pcap is exported in .csv file
#########################################################################
